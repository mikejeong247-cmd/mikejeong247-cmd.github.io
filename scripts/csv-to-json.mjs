...
const ROWS_PER_SHEET = parseInt(process.argv.find(a=>a.startsWith('--rows='))?.split('=')[1] || '13',10);
...
async function main(){
  ...
  const uniqSeqs=[...new Set(rows.map(r=>normSeq(r.code,r.emoji)).filter(Boolean))];
  const limit=16; let idx=0, inFlight=0;
  await new Promise(resolve=>{
    const kick=()=>{ while(inFlight<limit && idx<uniqSeqs.length){ const s=uniqSeqs[idx++]; inFlight++;
      (async()=>{ const p=await bestProvider(s); if(p==='twemoji') support.twemoji.push(s); else if(p==='openmoji') support.openmoji.push(s); })()
        .finally(()=>{ inFlight--; if(idx>=uniqSeqs.length && inFlight===0) resolve(); else kick(); });
    }}; kick();
  });
  ...
  for(const r of rows){
    ...
    if(support.twemoji.includes(s) || support.openmoji.includes(s)) pool.push({ seq:s, provider: support.twemoji.includes(s)?'twemoji':'openmoji' });
    if(pool.length>=TOP_N) break;
  }

  const items=[];
  const pngBuf=async(seq,provider)=>{ const u=provider==='twemoji'?TW_PNG(seq):OM_PNG(seq); const r=await fetch(u); if(!r.ok) return null; return Buffer.from(await r.arrayBuffer()); };
  idx=0; inFlight=0;
  await new Promise(resolve=>{
    const kick=()=>{ while(inFlight<12 && idx<pool.length){ const cur=pool[idx++]; inFlight++;
      (async()=>{ const buf=await pngBuf(cur.seq,cur.provider); if(buf){ const png=await sharp(buf).resize(TILE,TILE,{fit:'contain',background:{r:0,g:0,b:0,alpha:0}}).png({quality:90}).toBuffer(); items.push({key:cur.seq,png}); } })()
  ...
  while(pos<items.length){
    const sheetItems=items.slice(pos,pos+COLS*ROWS_PER_SHEET);
    const rowsCount=Math.ceil(sheetItems.length/COLS);
    const width=COLS*TILE, height=rowsCount*TILE;
    const composites=[];
    sheetItems.forEach((it,i)=>{ const x=(i%COLS)*TILE, y=Math.floor(i/COLS)*TILE; composites.push({input:it.png,left:x,top:y}); map[it.key]={sheet:`emoji-top${TOP_N}-${sheetIdx}.webp`,x,y,w:TILE,h:TILE}; });
    const out=path.join(OUT_DIR,`emoji-top${TOP_N}-${sheetIdx}.webp`);
    await sharp({create:{width,height,channels:4,background:{r:0,g:0,b:0,alpha:0}}}).composite(composites).webp({quality:82}).toFile(out);
    sheetIdx++; pos+=sheetItems.length;
  }
